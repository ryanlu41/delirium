...
Pulling medication data from MIMIC-IV. Looking in prescriptions, 
inputevents, pharmacy, and ingredient events.
...
def TwentyFourHourDrugFeature(drugSearchListPath):
    ...
    comp = pd.read_csv(dataset_path.joinpath('MIMICIV_complete_dataset.csv'),
                       usecols = ['hadm_id', 'stay_id','delirium_pos','intime',
    ...
    comp_data = comp_data[comp_data['startoffset'] <= 1440]
...
#%% Testing
import time
drugSearchListPath=r'C:\Users\Kirby\OneDrive\JHU\Precision Care Medicine\First 24 Hour\Features\Medications\DrugNameLists\Vasopressors.csv'
start = time.time()
